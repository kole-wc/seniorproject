...
namespace pms
{
    public partial class FormReportBS : Form
    {
        ...
        private void FormReportBS_Load(object sender, EventArgs e)
        {
        ...
        private void LoadData()
        {
            ...
            string sql = "SELECT sales.id, order_details.sales_id, order_details.product_id, order_details.name, order_details.type, SUM(order_details.quantity), SUM(order_details.price), sales.order_date FROM order_details " +
                         "INNER JOIN sales ON sales.id = order_details.sales_id WHERE order_date BETWEEN 'd1' AND 'd2' GROUP BY product_id ORDER BY SUM(quantity) DESC";
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            m1.ExecuteQuery(sql);
            new FormExportReportBS().ShowDialog();
        }
